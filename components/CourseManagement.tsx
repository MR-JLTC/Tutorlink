...
const CourseManagement: React.FC = () => {
    ...
    const [form, setForm] = useState<{ course_name: string; university_id: number; subject_name?: string; semester?: string } | null>(null);
    ...
    const addSubject = async (courseId: number) => {
        ...
        await apiClient.post(`/courses/${courseId}/subjects`, { subject_name: form.subject_name, semester: form.semester });
        const response = await apiClient.get(`/courses/${courseId}/subjects`);
        setCourses(prev => prev.map(c => c.course_id === courseId ? { ...c, subjects: response.data } as CourseWithDetails : c));
        setForm(prev => prev ? { ...prev, subject_name: '', semester: '' } : prev);
    }
    ...
    const openEditSubject = (subject: Subject) => {
        ...
        setForm(prev => ({ course_name: '', university_id: 0, subject_name: subject.subject_name, semester: (subject as any).semester || '' }));
    }

    const saveSubject = async (courseId: number) => {
        if (!editSubject || !form) return;
        await apiClient.patch(`/courses/${courseId}/subjects/${editSubject.subject_id}`, { subject_name: form.subject_name, semester: form.semester });
        const response = await apiClient.get(`/courses/${courseId}/subjects`);
        setCourses(prev => prev.map(c => c.course_id === courseId ? { ...c, subjects: response.data } as CourseWithDetails : c));
        setEditSubject(null);
        setForm(prev => prev ? { ...prev, subject_name: '', semester: '' } : prev);
                                        ...
                                        <tr>
                                            <td colSpan={4} className="p-0">
                                                <div className="px-6 py-4 bg-gray-50">
                                                    ...
                                                    <div className="ml-12 mb-3 grid grid-cols-1 md:grid-cols-4 gap-2 items-center">
                                                        <input className="border border-slate-300 rounded px-2 py-1" placeholder="New subject name" value={form?.subject_name || ''} onChange={(e) => setForm(prev => prev ? { ...prev, subject_name: e.target.value } : prev)} />
                                                        <input type="number" min={1} step={1} className="border border-slate-300 rounded px-2 py-1" placeholder="Semester (number)" value={form?.semester || ''} onChange={(e) => setForm(prev => prev ? { ...prev, semester: e.target.value.replace(/[^0-9]/g, '') } : prev)} />
                                                    ...
                                                    {course.subjects && course.subjects.length > 0 ? (
                                                        <ul className="list-disc pl-16 space-y-1">
                                                            {[...(course.subjects || [])]
                                                                .sort((a, b) => {
                                                                    const sa = Number((a as any).semester) || 0;
                                                                    const sb = Number((b as any).semester) || 0;
                                                                    if (sa !== sb) return sa - sb;
                                                                    return a.subject_name.localeCompare(b.subject_name);
                                                                })
                                                                .map(subject => (
                                                                <li key={subject.subject_id} className="text-sm text-gray-700 flex items-center justify-between">
                                                                    <span>{subject.subject_name}{(subject as any).semester ? ` — ${(subject as any).semester}` : ''}</span>
                    ...
                    {form && (
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="number" min={1} step={1} className="mt-1 block w-full border border-slate-300 rounded-md px-3 py-2" value={form.semester || ''} onChange={(e) => setForm(prev => prev ? { ...prev, semester: e.target.value.replace(/[^0-9]/g, '') } : prev)} />
                            </div>
                        </div>
                    )}
                </Modal>
