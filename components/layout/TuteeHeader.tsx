    ...
    <header className="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-30">
      <div className="px-2 sm:px-3 md:px-4 lg:px-4 py-2 sm:py-2.5 md:py-4 lg:py-4 md:h-[72px] lg:h-[72px] md:flex md:items-center">
        <div className="flex items-center justify-between gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 w-full md:h-full">
          <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 flex-1 min-w-0 md:h-full">
            {/* Mobile Menu Button */}
            <button
              onClick={onMenuClick}
            ...
            <div className="min-w-0 flex-1 md:flex md:items-center md:h-full">
              <div className="leading-none">
                <h1 className="text-sm sm:text-base md:text-lg lg:text-lg font-semibold md:font-bold text-slate-800 truncate leading-tight">
                  <span className="truncate inline-block max-w-full">Welcome back, {user?.name?.split(' ')[0] || 'Student'} </span>
                </h1>
                <p className="text-[10px] sm:text-xs md:text-xs text-slate-600 md:text-slate-600 md:font-medium hidden sm:block break-words leading-tight mt-0">
