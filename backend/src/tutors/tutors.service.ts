...
export class TutorsService {
  constructor(
    ...
    @InjectRepository(AvailabilityChangeRequest)
    private availabilityChangeRequestRepository: Repository<AvailabilityChangeRequest>,
  ...
  async saveProfileImage(tutorId: number, file: any) {
    const tutor = await this.tutorsRepository.findOne({ where: { user: { user_id: tutorId } } });
    if (!tutor) throw new Error('Tutor not found');

    // If no file uploaded, create placeholder
    if (!file) {
      const placeholderUrl = `/tutor_documents/tutorProfile_${tutorId}`;
    ...
    // Update database with new file URL first
    ...
    await this.deleteOldProfileImages(tutorId);
  ...
  async saveGcashQR(tutorId: number, file: any) {
    ...
    // If no file uploaded, create placeholder
    if (!file) {
      const placeholderUrl = `/tutor_documents/gcashQR_${tutorId}`;
    ...
    await this.deleteOldGcashQRFiles(tutorId);
  ...
  async getAvailabilityChangeRequests(userId: number) {
    const tutor = await this.tutorsRepository.findOne({ 
      where: { user: { user_id: userId } },
      relations: ['user']
    });
    ...
    const requests = await this.availabilityChangeRequestRepository.find({
      where: { tutor: { tutor_id: tutor.tutor_id } as any },
      order: { created_at: 'DESC' }
    });
    return requests;
  ...
  async submitAvailabilityChangeRequest(userId: number, data: {
    day_of_week: string;
    start_time: string;
    end_time: string;
    reason: string;
    ...
    const request = this.availabilityChangeRequestRepository.create({
      tutor,
      ...data,
      status: 'pending'
    });
    await this.availabilityChangeRequestRepository.save(request);
  ...
  private async deleteOldProfileImages(tutorId: number) {
    const tutor = await this.tutorsRepository.findOne({ where: { user: { user_id: tutorId } } });
    if (!tutor) return;
    ...
    try {
      ...
      // Find files that match the profile image pattern for this tutor
      const profileImagePattern = new RegExp(`^tutorProfile_${tutorId}(\\..*)?$`);
      ...
      console.log(`Found ${filesToDelete.length} old profile image files to delete for tutor ${tutorId}:`, filesToDelete);
  ...
  private async deleteOldGcashQRFiles(tutorId: number) {
    const tutor = await this.tutorsRepository.findOne({ where: { user: { user_id: tutorId } } });
    if (!tutor) return;
    ...
    try {
      ...
      // Find files that match the GCash QR pattern for this tutor
      const gcashQRPattern = new RegExp(`^gcashQR_${tutorId}(\\..*)?$`);
      const filesToDelete = files.filter(file => {
      ...
      console.log(`Found ${filesToDelete.length} old GCash QR files to delete for tutor ${tutorId}:`, filesToDelete);
